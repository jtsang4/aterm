...
5. For connection features, prove remote reality with target-specific command output through the visible shipped session UI (`MainActivity`/navigation path), not through `setContent(...)`-mounted screen substitutes or coordinator-only state. For relaunch/restart validation, use a real activity/app relaunch path rather than swapping a fresh container into the same Compose root. For terminal features, do not hand off the work as complete while the UI is still transcript-backed instead of live PTY-driven; return partial progress to the orchestrator instead.
...
7. Manually exercise lifecycle and terminal truthfulness whenever the feature touches connect/disconnect, resize, rotation, backgrounding, special keys, paste, or snippet dispatch to a live session. These observations must be recorded explicitly in `handoff.verification.interactiveChecks`; missing interactive evidence is a procedure deviation. For full terminal claims, use live-fixture evidence rather than fake session-controller substitutes.
