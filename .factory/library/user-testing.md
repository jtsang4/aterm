...
- Timeout-diagnosis rerun note: the saved-host/session proof must explicitly use `/data/local/tmp/aterm-fixture-client_key_legacy_pem` for the encrypted imported-key path and must capture whether failure occurs before TCP connect, during `session.auth()`, or during shell-channel open; a generic timeout is not sufficient evidence.
- Legacy imported-key session note: `SessionSshFixtureInstrumentedTest#imported_key_identity_survives_relaunch_and_reaches_real_terminal_session` is currently a diagnostic proof, not a pure pass/fail contract oracle. The test can finish green while logcat still reports `legacy imported-key ui outcome state=FAILED status=Connection timed out while reaching 10.0.2.2:3122.` after a successful `legacy imported-key probe tcp=true auth=true shell=true`. For `VAL-CROSS-001`, inspect both lines before claiming the shipped app-shell path passed.
